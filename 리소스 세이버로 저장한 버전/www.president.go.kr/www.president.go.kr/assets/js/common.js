...
var ui = {
    ...
    contentSet: function() {
        ...
        function scrollToNextLine(stopEvent) {
            ...
            if (!((navigator.appName == 'Netscape' && agent.indexOf('trident') != -1) || (agent.indexOf("msie") != -1))) {
                /* 크롬의 경우 휠 기능 제거 */
                document.querySelector(".subscribe.video").addEventListener("wheel", stopEvent, {
                    passive: false
                });
            }
    ...
    Accordion: {
        ...
        click: function(target) {
            if ($(target).parents("li").hasClass("on")) {
                ...
                $(target).parents("li").find(".AccordionBtn").attr({
                    "aria-expanded": true,
                    "title": "확장됨"
                });
    ...
    LayerPop: {
        ...
        Show: function(PopID, PopSize, open_callbackfcn, open_Pram, closed_callbackfcn, closed_Pram) {
            ...
            if ($(PopID).find("a, button, input, select").length == 1) {
                $(PopID).find(".popLayout").attr({
                    "tabindex": "0",
                    "data-pop-focus": "first"
                });
            }
    ...
    select: {
        ...
        action: {
            open: function(target) {
                ...
                $(target).parents(".selectBase").find(".toggleBtn").attr({
                    "aria-expanded": true
                });
    ...
    contAutoHeight: function(target, point) {
        ...
        if (point[0][1] != undefined) {
            tabletPoint = point[0][1]
        }
        if (point[1][1] != undefined) {
            mobilePoint = point[1][1]
        }
